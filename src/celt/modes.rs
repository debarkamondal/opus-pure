...
/// Samples in one short MDCT block of the 48 kHz mode (2.5 ms). Every valid
/// CELT frame size is this shifted left by the frame's `lm`.
pub const SHORT_MDCT_SIZE: usize = 120;

/// Largest `lm` the 48 kHz mode supports: frame sizes run up to
/// `SHORT_MDCT_SIZE << MAX_LM` = 960 samples (20 ms at 48 kHz).
...
impl CeltMode {
    pub fn new_48000_960_120() -> Self {
        let short_mdct_size = SHORT_MDCT_SIZE;
